...
if __name__ == "__main__":
    ...
    resposta_build = (
        input("🐳 Deseja fazer o BUILD da nova imagem Docker agora? (S/n): ")
        .strip()
        .lower()
    )
    ...
    if resposta_build in ["", "s", "sim", "y", "yes"]:
        ...
        print("-" * 50)
        resposta_push = (
            input("☁️  Deseja fazer o PUSH para o Docker Hub agora? (S/n): ")
            .strip()
            .lower()
        )
        ...
        print(
            "Build cancelado. Os textos foram atualizados, mas a imagem do Docker não foi regerada."
        )
